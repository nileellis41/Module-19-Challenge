...
# Streamlit sidebar for account generation
...
# Display Ethereum balance in the sidebar
...
# Select a fintech professional from the dropdown
person = st.sidebar.selectbox("Select a Person", people)

# Input the amount of time the professional was hired for
hours = st.sidebar.number_input("Number of Hours", min_value=0.0, format="%.2f")

# Fetch the candidate's hourly rate from the database
hourly_rate = candidate_database[person][3]

# Calculate the total wage
wage = hours * hourly_rate

# Display the calculated wage in the sidebar
st.sidebar.write(f"Total Wage in Ether: {wage:.3f} ETH")



# @TODOs
# Write the `wage` calculation to the Streamlit sidebar
Wage = st.sidebar.selectbox('Wage Calculation', kitties)
